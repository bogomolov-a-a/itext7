...
public class GposLookupType8 extends GposLookupType7 {
    ...
    private static final long serialVersionUID = 297527612671201181L;
    ...
    protected void readSubTableFormat2(int subTableLocation) throws java.io.IOException {
        ...
        List<List<ContextualPositionRule>> posClassSets = new ArrayList<>(chainPosClassSetCount);
        for (int i = 0; i < chainPosClassSetCount; ++i) {
            List<ContextualPositionRule> posClassSet = null;
            if (chainPosClassSetOffsets[i] != 0) {
                ...
                for (int j = 0; j < chainPosClassRuleCount; ++j) {
                    ...
                    rule = new PosRuleFormat2(t, backtrackClassIds, inputClassIds, lookAheadClassIds, posLookupRecords);
                    posClassSet.add(rule);
                }
            }
            posClassSets.add(posClassSet);
        }

        t.setPosClassSets(posClassSets);
        subTables.add(t);
    }
