...
final class SignUtils {
  ...
  static byte[] getExtensionValueByOid(X509Certificate certificate,
                                       String oid) {
  ...
  static MessageDigest getMessageDigest(String hashAlgorithm,
                                        IExternalDigest externalDigest) throws GeneralSecurityException {
  ...
  static ICertificateID generateCertificateId(X509Certificate issuerCert,
    ...
    return FACTORY.createCertificateID(
      FACTORY.createJcaDigestCalculatorProviderBuilder()
        .build()
  ...
  static ICertificateID generateCertificateId(X509Certificate issuerCert,
    ...
    return FACTORY.createCertificateID(
      FACTORY.createJcaDigestCalculatorProviderBuilder()
        .build()
        .get(
          FACTORY.createAlgorithmIdentifier(identifier,
  ...
  static IOCSPReq generateOcspRequestWithNonce(ICertificateID id) throws IOException, AbstractOCSPException {
    ...
    IDEROctetString derOctetString = FACTORY.createDEROctetString(
      FACTORY.createDEROctetString(PdfEncryption.generateNewDocumentId())
        .getEncoded());
    IExtension ext = FACTORY.createExtension(
      FACTORY.createOCSPObjectIdentifiers()
        .getIdPkixOcspNonce(),
      false,
      derOctetString);
  ...
  static InputStream getHttpResponseForOcspRequest(byte[] request,
                                                   URL urlt) throws IOException {
    HttpURLConnection con = (HttpURLConnection) urlt.openConnection();
  ...
  static boolean isSignatureValid(IBasicOCSPResp validator,
                                  Certificate certStoreX509,
                                  String provider)
  ...
  static void isSignatureValid(ITimeStampToken validator,
    ...
    ISignerInformationVerifier verifier = FACTORY.createJcaSimpleSignerInfoVerifierBuilder()
      .setProvider(provider)
      .build(certStoreX509);
    validator.validate(verifier);
  }

  static boolean checkIfIssuersMatch(ICertificateID certID,
  ...
  static Collection<Certificate> readAllCerts(byte[] contentsKey,
                                              CertificateFactory externalCertificateFactory) throws CertificateException {
    final CertificateFactory factory = externalCertificateFactory == null ? CertificateFactory.getInstance("X509",
      FACTORY.getProvider()) : externalCertificateFactory;
  ...
  static X500Principal getIssuerX500Principal(IASN1Sequence issuerAndSerialNumber) throws IOException {
    return new X500Principal(issuerAndSerialNumber.getObjectAt(0)
      .toASN1Primitive()
      .getEncoded());
  }
  ...
  static TsaResponse getTsaResponseForUserRequest(String tsaUrl,
                                                  byte[] requestBytes,
                                                  String tsaUsername,
    ...
    if ((tsaUsername != null) && !tsaUsername.equals("")) {
      ...
      tsaConnection.setRequestProperty("Authorization",
        "Basic " +
          Base64.encodeBytes(userPassword.getBytes(StandardCharsets.UTF_8),
  ...
  static Signature getSignatureHelper(String algorithm,
                                      String provider)
  ...
  static void setRSASSAPSSParamsWithMGF1(Signature signature,
  ...
  public static void updateVerifier(Signature signature,
                                    byte[] attr) throws SignatureException {
    signature.update(attr);
  }

  static boolean verifyCertificateSignature(X509Certificate certificate,
